...
public class PostController {
    ...
    // Bean에 해당하는 정보를 변수 이름인 postRepository로 사용하겠다 뜻입니다.
    @Autowired
    ...
    public Post returnPost() {
        ...
        //위와 같이 만들어진 정보를 리턴
        // "title" 이라는 key값에 "제목" 이라는 value값을 리턴
        // "content" 라는 key값에 "내용" 이라는 value값을 리턴
        // 위의 형식이 JSON 형태라는 것입니다.
    }
    // @PostMapping은 POST 방식이라고 해서 JSON 형태로 데이터를 처리
    // 그렇기 때문에 URL에 정보 노출이 안된다는 이점이 있음
    // 웹 브라우저로는 Post 요청을 할 방법이 없기 때문에 postman을 사용
    // @RequestBody를 통해 JSON 형태의 입력 (파라미터) 을 수신
    @PostMapping("/create")
    public Post createPost(@RequestBody PostRequest request) {
        // 포스트맨에서 그냥 http://localhost:8080/create으로 요청하면 오류가 뜸
        /*
            {
                "timestamp": "2025-07-04T01:27:33.611+00:00",
                "status": 400,
                "error": "Bad Request",
                "path": "/create"
            }

          그래서 포스트맨의 바디에 raw,JSON으로 설정하고 아래 내용을 추가해서 보냅니다.
            {
                "title": "안녕",
                "content": "내용"
            }
            ...
            전부 비어버린 이유는 request가 null 처리되었지 때문입니다.

         */

        // 중괄로 '{}' 내부에 'request' 정보가 배치됩니다.
        ...
        // 직접 생성자가 만든 객체와 set 을 사용해서 정보들을 배치해주기 때문입니다.
...
//        return requestedPost;
    ...
    // GetMapping 내무에'{id}' 형태로 표기된 것이 존재합니다.
    // @PathVariable 의 경우 가변 인자에 해당합니다.
    // id 값이 562번, 768qjs, 1237895번
    ...
    public List<Post> listPost(){
        log.info("post list -> request");
        return postRepository.findAll();
    }

    @GetMapping("post/delete")
    public void postDelete(@RequestParam Long id){
